...
def _normalize_table_identifier(token: str) -> str:
    cleaned = token.strip().strip("`").strip("[]").strip('"')
    parts = [part.strip("`[]\"") for part in cleaned.split(".") if part]
    return ".".join(parts).lower()


def _extract_table_tokens(statement: str) -> List[str]:
...
def _extract_select_columns_simple(statement: str) -> List[str]:
...
def _validate_sql_statement(
    ...
    *,
    ...
    if table_metadata:
        ...
        # Disallow multiple tables/joins/unions in this simple validator.
        if len(set(_normalize_table_identifier(t) for t in table_tokens)) > 1:
            return "현재는 단일 테이블만 조회할 수 있습니다. 여러 테이블/UNION/조인은 지원하지 않습니다."
        allowed_tables: set[str] = set()
        columns_by_table: Dict[str, set[str]] = {}
        for tname, meta in table_metadata.items():
            norm_key = _normalize_table_identifier(tname)
            allowed_tables.add(norm_key)
            meta = meta or {}
            full_name = meta.get("full_name")
            if full_name:
                allowed_tables.add(_normalize_table_identifier(full_name))
            col_set = {
            ...
            if col_set:
                ...
                if full_name:
                    columns_by_table[_normalize_table_identifier(full_name)] = col_set
        if table_tokens:
            for token in table_tokens:
                norm_token = _normalize_table_identifier(token)
                if norm_token not in allowed_tables:
                    return f"존재하지 않는 테이블을 참조합니다: {token}"
            if len(table_tokens) == 1:
                norm_token = _normalize_table_identifier(table_tokens[0])
                col_set = columns_by_table.get(norm_token)
                if col_set:
                    select_columns = _extract_select_columns_simple(normalized)
